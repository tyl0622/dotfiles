...
typeset -ga JOVIAL_PROMPT_ORDER=( path dev-env git-info )
...
@jov.set-typing-pointer() {
    ...
    if [[ -n ${jovial_rev_git_dir} ]]; then
        ...
        jovial_parts[typing]+="${JOVIAL_PALETTE[arrow]}${JOVIAL_SYMBOL[arrow]}"
    fi
}
...
@jov.set-current-dir() {
    ...
    home_dir="$HOME"
    last_dir="${full_path##*/}"
    path_wo_last_dir="${full_path%/*}"
    short_path="${path_wo_last_dir/$home_dir/~}"

    jovial_parts[path]="${(%):-${JOVIAL_PALETTE[path_dir]}${short_path}/${JOVIAL_PALETTE[path_base]}${last_dir}}"
    ...
    jovial_parts[path]="${JOVIAL_AFFIXES[path.prefix]}${JOVIAL_PALETTE[path_dir]}$jovial_parts[path]${JOVIAL_AFFIXES[path.suffix]}"
}
